...
export const PublicStatsBillingResponseSchema = z
  .object({
    ...
    first_payment_times_unix: z.array(z.number().int()).openapi({
      description:
        "Every account's FIRST settled payment, unix SECONDS, ascending — one entry per account that has ever paid, so `first_payment_times.length === total_paying_accounts`. This is how you answer a ROLLING window EXACTLY: the number of accounts that became customers in the last N days is `first_payment_times.filter(t => t >= nowUnixSeconds - N * 86400).length`. Do NOT try to get that by summing `first_time_paying_accounts` over buckets — those are calendar months and weeks, a rolling window aligns to neither, and the bucket straddling the window's edge carries payments on both sides of it (measured against production at 90 days, whole-bucket summing read 17 where the truth was 25). Same identity, same acquirer coverage and same settled-only predicates as `total_paying_accounts`, so the two can never tell different stories; a payment with no resolvable org has no entry here, exactly as it has no account in the counts. Counting the whole array reproduces `total_paying_accounts`. No money is published at this grain — only when each account started paying.\n\nTHE UNIT IS IN THE NAME deliberately: `Date.now()` is MILLISECONDS, so a consumer comparing a milliseconds cutoff against these seconds counts ZERO and renders a dash — the exact failure this array exists to kill. Every money field here says `_cents`; this one said nothing until the rename.",
    }),
    first_payment_times: z.array(z.number().int()).openapi({
      description:
        "DEPRECATED — identical array, same order, same semantics as `first_payment_times_unix`, served one more release so the rename breaks no consumer. Read `first_payment_times_unix` instead: it says its unit, and reading unix SECONDS as milliseconds is the one mistake that turns this field back into the dash it was built to remove.",
    }),
