...
- **src/lib/event-processor.ts** — Idempotent event ingestion + Stripe-shape table upserts (shared by webhook handler and event-poller). Also dispatches post-mirror side-effects (default-PM promotion, Stripe-fee declaration) via the lazily-cached platform Stripe client.
