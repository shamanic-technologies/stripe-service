...
- **Boot-time historical back-fill.** On every boot, `backfillHistorical()` runs as a fire-and-forget background task fired right after `app.listen()`. Iterates `customers.list`, `paymentIntents.list`, `checkout.sessions.list` globally, and `customers.listBalanceTransactions(cus_X)` per customer in DB. Routes each object to its tenant via `metadata.org_id` (falls back to `"unknown"` if missing). Idempotent via `ON CONFLICT DO UPDATE` so re-runs refresh `raw_json` / status. Failure is logged via `console.error` but does NOT crash the service — the port is bound first so callers never get ECONNREFUSED during the back-fill window (CBT alone can be 10k+ rows, exceeding Railway's boot tolerance). Recovers Feb→Apr 2026 history that the 30-day events.list cursor cannot reach.
